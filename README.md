...
A full-stack app for students and interns to log daily work, track hours toward a **468-hour** requirement, and produce a reflection report. It combines a Pomodoro timer, a calendar, daily journal entries, and AI-assisted structuring (ARAS) so users can meet program compliance and build a single compiled report for submission.

---

## What it does

- **Tracks internship hours** against a 468-hour target. Progress (total hours, remaining, days completed, streaks) is computed from finished journal days and shown on a dashboard and calendar.
...
- **Compiled report:** Once 468 hours are reached, the user can compile a PDF report (name, company, date range, total hours, and ARAS per entry) for submission to school or employer.
- **Export:** Journal data can be exported as JSON or Markdown for backup or external use.

The app enforces a single target (468 hours) everywhere: database default, server constant, and client fallback. When the user reaches 468 finished hours, new journal entries are blocked and the focus shifts to compiling and downloading the report.
...
├── shared/
│   └── constants.js        # TARGET_HOURS (468) used by server
└── supabase/
    ├── schema.sql          # profiles, journals, internship_progress, ai_logs, compiled_reports, RLS, triggers
    └── migrations/         # 001 finish_journal_day, 002 target_hours_468
```
...
5. **Compilation:** When total hours ≥ 468, the user can compile a report (date range, all finished entries with ARAS) and download it as PDF.
...
- **Target:** 468 hours (defined in `shared/constants.js` and `client/src/lib/constants.js`; DB default in `profiles.target_hours` and in `recalculate_progress()`).
